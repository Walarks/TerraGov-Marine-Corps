...
/datum/action/xeno_action/activable/emit_neurogas/use_ability(atom/A)
	...
	if(!do_after(X, DEFILER_GAS_CHANNEL_TIME, TRUE, null, BUSY_ICON_HOSTILE))
		if(!QDELETED(src))
			to_chat(X, "<span class='xenodanger'>We abort emitting gas, our expended plasma resulting in nothing.</span>")
...
/datum/action/xeno_action/select_reagent
	...
	mechanics_text = "Selects which reagent to use for reagent slash and gas. Hemodile slows by 25%, increased to 50% with \
